													...
													t_float_gpu element_pitch, t_float_gpu M_elements, int N_elements )
{
	...
	int bandwidth_cut = 0;
	float bandwidth_over_two = ( N_samples_dft_bp - 1 ) / 2.0f;
	...
	for( int index_block_f = 0; index_block_f < N_blocks_Omega_bp; index_block_f++ )
	{
		...
		// {
				...
				// boundaries of actual receive aperture
				...
				if( ( index_rx < index_aperture_lb ) || ( index_rx > index_aperture_ub ) )
				{
					if( bandwidth_cut == 0 )
					{
						// reduce bandwidth for current channel
						bandwidth_over_two = ( t_float_gpu ) index_f_in_band / 2.0f;
						bandwidth_cut = 1;
					}
					break;
				}
	...
	for( int index_block_f = 0; index_block_f < N_blocks_Omega_bp; index_block_f++ )
	{
		...
		for( index_f_in_block = 0; index_f_in_block < N_THREADS_PER_BLOCK; index_f_in_block++ )
		{
			...
			if( l_x < N_pos_lat_x && l_z < N_pos_lat_z )
			{
				...
				{
					apodization = apodization * apply( window_f, index_f_in_band - bandwidth_over_two, bandwidth_over_two ) / apodization_sum;
				}
				else
				{
					apodization = 0.0f;
	...
	//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	// write results into global memory
	//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	if( l_x < N_pos_lat_x && l_z < N_pos_lat_z )
	{
